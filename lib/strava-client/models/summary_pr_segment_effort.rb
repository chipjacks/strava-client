  ...
  class SummaryPRSegmentEffort
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `StravaClient::SummaryPRSegmentEffort` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `StravaClient::SummaryPRSegmentEffort`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'pr_activity_id')
        self.pr_activity_id = attributes[:'pr_activity_id']
      end

      if attributes.key?(:'pr_elapsed_time')
        self.pr_elapsed_time = attributes[:'pr_elapsed_time']
      end

      if attributes.key?(:'pr_date')
